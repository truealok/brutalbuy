...
/* ---------------- runtime config ----------------
   Injected by layout/theme.liquid before these scripts run, so every asset URL
   and internal route is resolved by Liquid instead of being hardcoded. */
BB.cfg = window.BB_CONFIG || {};
BB.url = BB.cfg.routes || {};
BB.assets = BB.cfg.assets || {};

/* The original static site used flat .html files, and bb-data.js / bb-home.js /
   bb-catalog.js / bb-product.js / bb-cart.js still emit those paths.
   Rather than rewrite every template literal, translate at interaction time. */
var BB_LEGACY_ROUTES = {
  "index.html": function () { return BB.url.home || "/"; },
  "catalog.html": function () { return BB.url.catalog || "/pages/catalog"; },
  "product.html": function () { return BB.url.product || "/pages/product"; },
  "cart.html": function () { return BB.url.cart || "/pages/cart"; },
  "brands.html": function () { return BB.url.brands || "/pages/brands"; },
  "b2b.html": function () { return BB.url.b2b || "/pages/b2b"; },
  "about.html": function () { return BB.url.about || "/pages/about"; },
  "contact.html": function () { return BB.url.contact || "/pages/contact"; },
  "faqs.html": function () { return BB.url.faqs || "/pages/faqs"; },
  "account.html": function () { return BB.url.account || "/pages/account"; },
  "withdrawal.html": function () { return BB.url.withdrawal || "/pages/withdrawal"; }
};

BB.fixHref = function (href) {
  if (!href) return href;
  /* [a-z0-9-] not [a-z-] — "b2b.html" contains a digit. */
  var m = /^([a-z0-9-]+\.html)(\?[^#]*)?(#.*)?$/i.exec(href);
  if (!m || !BB_LEGACY_ROUTES[m[1]]) return href;
  return BB_LEGACY_ROUTES[m[1]]() + (m[2] || "") + (m[3] || "");
};

/* Resolve legacy links on hover (so the rewritten URL shows and opens correctly
   in a new tab) and again on click as a guaranteed fallback. */
document.addEventListener("mouseover", function (e) {
  var a = e.target && e.target.closest ? e.target.closest("a[href]") : null;
  if (!a) return;
  var fixed = BB.fixHref(a.getAttribute("href"));
...
document.addEventListener("click", function (e) {
  ...
  if (!a) return;
  var fixed = BB.fixHref(a.getAttribute("href"));
  if (fixed === a.getAttribute("href")) return;
  e.preventDefault();
...
/* Reads the JSON blob a section wrote into data-bb-settings, so customizer
   values can override copy that this JS generates. */
BB.sectionSettings = function (id) {
  var el = document.getElementById(id);
  if (!el || !el.dataset.bbSettings) return {};
  try { return JSON.parse(el.dataset.bbSettings) || {}; } catch (e) { return {}; }
};
...
function headerHTML() {
  /* The announcement bar lives in sections/announcement-bar.liquid so it can be
     edited in the customizer; only the header itself is built here. */
  return `
  <header class="header">
    <div class="container header-inner">
      <button class="hamburger" id="hamburger" aria-label="Open menu"><span></span><span></span><span></span></button>
      <a class="logo" href="index.html">
        <img src="${BB.assets.logoDark}" alt="Brutal Buy" id="logoImg">
...
function accountHTML() {
  ...
  <aside class="drawer drawer-right" id="accountDrawer" aria-label="Account">
    ...
    <div class="drawer-body">
      <button class="acc-drawer-btn" onclick="location.href='${BB.url.account || "/pages/account"}'">
      ...
      <form class="field" onsubmit="event.preventDefault();location.href='${BB.url.account || "/pages/account"}'">
  ...
  <footer class="footer">
    <div class="container">
      <div class="footer-grid">
        <div>
          <a class="logo" href="index.html"><img class="f-logo" src="${BB.assets.logoDark}" alt="Brutal Buy"></a>
...
function initAnnounce() {
  ...
  const speed = (parseInt(host && host.dataset.speed, 10) || 3.8) * 1000;
  let i = 0;
  setInterval(() => {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const applyLogo = () => {
    ...
    document.querySelectorAll("#logoImg, .f-logo").forEach(img => {
      img.src = dark ? (BB.assets.logoDark || "assets/logo-dark.svg") : (BB.assets.logoLight || "assets/logo.svg");
    });
  };
  applyLogo();
  document.querySelectorAll(".theme-toggle").forEach(t =>
