...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (deal) {
    const ds = BB.sectionSettings("dealBanner");
    const dCta1 = (ds.cta1 && ds.cta1.href) || "catalog.html?tag=sale";
    const dCta2 = (ds.cta2 && ds.cta2.href) || "catalog.html?sport=cricket&cat=bats";
    deal.innerHTML = `
      <div class="deal reveal">
        <div class="deal-inner">
          <span class="hero-tag">${ds.tag || "Deal of the Week"}</span>
          <h2 class="section-title" style="margin:14px 0 8px;font-size:clamp(28px,3.6vw,42px)">${ds.heading || "Pro Kit — Up to 45% Off"}</h2>
          <p style="color:var(--text-dim);max-width:460px">${ds.subheading || "Match-grade gear bundles for cricket, football and court sports. When it's gone, it's gone."}</p>
          ...
          <div style="display:flex;gap:12px;margin-top:26px;flex-wrap:wrap">
            <a class="btn btn-primary" href="${dCta1}">${(ds.cta1 && ds.cta1.label) || "Shop the deal"}</a>
            <a class="btn btn-ghost" href="${dCta2}">${(ds.cta2 && ds.cta2.label) || "View bats"}</a>
          </div>
        </div>
      </div>`;
  ...
  if (mq) {
    const ms = BB.sectionSettings("marquee");
    const items = (ms.items && ms.items.filter(Boolean).length) ? ms.items.filter(Boolean) : BB.marquee;
    const chunk = items.map(m => `<span>${BB.esc(m)}</span>`).join("");
  ...
  if (vb) {
    const vs = BB.sectionSettings("videoBanner");
    vb.innerHTML = `
      <div class="vb-inner">
        <h2>${vs.heading || "Here to Help. <em>Plays Hard.</em>"}</h2>
        <p style="color:var(--text-dim);margin-top:10px">${vs.subheading || "Watch how pros train with Brutal Buy gear"}</p>
        <button class="vb-play" aria-label="${BB.esc(vs.buttonLabel || "Play video")}" onclick="BB.toast('Video player is a demo ▶')">▶</button>
      </div>`;
  }
  ...
  if (nl) {
    const ns = BB.sectionSettings("newsletter");
    nl.innerHTML = `
      <div class="newsletter reveal">
        <div>
          <h3>${ns.heading || "Join the Brutal List ⚡"}</h3>
          <p>${ns.subheading || "Early drops, training tips and members-only pricing. No spam — just gear."}</p>
        </div>
        <form class="nl-form" data-success="${BB.esc(ns.success || "You are on the list! 🎉")}">
          <input type="email" name="contact[email]" placeholder="${BB.esc("Your email address")}" required>
          <button class="btn btn-primary">${ns.submit || "Subscribe"}</button>
        </form>
      </div>`;
    /* Demo mode: show the toast. Real mode: posted to Shopify by bb-pages.js. */
    nl.querySelector(".nl-form").addEventListener("submit", e => {
      if (e.defaultPrevented) return;
      e.preventDefault();
      BB.toast(e.currentTarget.dataset.success);
